...
jest.mock('../src/core/utils', () => {
  return {
    ...
    uploadRecordsToAlgolia: jest.fn().mockImplementation(
      async (client, indexName, _lang, records, method) => {
        await client[method]({
          indexName,
          objects: records
        });
...
describe('AlgoliaProvider', () => {
  ...
  describe('addObjects', () => {
    it('should load and process records from JSON files', async () => {
      ...
      expect(mockRun.glob).toHaveBeenCalledWith(
        '*-algolia.json',
  ...
  describe('config', () => {
    ...
    it('should include custom query settings if provided', () => {
      ...
      const providerWithQuerySettings = new AlgoliaProvider(mockRun as any, configWithQuerySettings);
